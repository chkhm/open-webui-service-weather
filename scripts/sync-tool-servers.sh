...
# array, so reordering would change tool ids.
...
# Merge inside the container, which has the database and sqlite3.
result=$(docker exec -i -e DESIRED="$desired" -e DRY_RUN="$dry_run" open-webui python3 - <<'PY'
...
printf '%s\n' "$result" | grep -E '^(STORED|ADD) ' | sed 's/^STORED /stored connections: /; s/^ADD /  + /'
